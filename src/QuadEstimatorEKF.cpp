...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  //method 1) create a rotation matrix based on your current Euler angles, integrate that, convert back to Euler angles
  /*
  float phi = rollEst;
  ...
  */
  ...
  //method 2) use the Quaternion<float> class, which has a handy FromEuler123_RPY function for creating a quaternion from Euler Roll/PitchYaw
  //(Quaternion<float> also has a IntegrateBodyRate function, though this uses quaternions, not Euler angles)
  ///*
  Quaternion<float> attitude = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  attitude.IntegrateBodyRate(gyro, dtIMU);
  V3D rpy = attitude.ToEulerRPY();
  float predictedRoll = rpy.x; //attitude.Roll();
  float predictedPitch = rpy.y; //attitude.Pitch();
